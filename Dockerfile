...
RUN BUILD_TIME=$(date "+%Y-%m-%d %H:%M:%S") && \
    ...
    go build -trimpath -buildvcs=false -tags "with_utls nomsgpack" -ldflags "-s -w -X 'github.com/yuanshuai1122/vodoge/internal/global.Version=${VERSION}' -X 'github.com/yuanshuai1122/vodoge/internal/global.BuildTime=${BUILD_TIME}'" -o /app/vodoge ./cmd/vodoge
