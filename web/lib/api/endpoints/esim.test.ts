...
describe("startDownloadProfile", () => {
  ...
  it("激活参数走请求体，URL 里不出现", async () => {
    fetchMock.mockResolvedValue(
      jsonResponse(202, { status: "ok", task_id: "t1" }),
    );
  ...
  it("409 但没有 task_id 时仍然抛错", async () => {
    fetchMock.mockResolvedValue(
      jsonResponse(409, { status: "error", message: "冲突" }),
    );
    await expect(
      startDownloadProfile("dev1", { smdp: "x" }),
  ...
  it("其它错误照常抛出", async () => {
    fetchMock.mockResolvedValue(
      jsonResponse(404, { status: "error", message: "设备未找到" }),
    );
    await expect(
      startDownloadProfile("dev1", { smdp: "x" }),
  ...
  it("设备 id 做 URL 编码", async () => {
    fetchMock.mockResolvedValue(jsonResponse(202, { task_id: "t" }));
