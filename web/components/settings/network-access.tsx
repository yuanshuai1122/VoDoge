    ...
    <Card>
      ...
      <CardContent className="flex flex-col gap-4">
        {query.isPending || !query.data ? (
          ...
          <NetworkAccessForm data={query.data} />
        )}
      </CardContent>
    </Card>
  );
...
function NetworkAccessForm({ data }: { data: SecuritySettings }) {
  const queryClient = useQueryClient();
  const [mode, setMode] = useState(() => data.mode || "internal");
  const [cidrs, setCidrs] = useState<string[]>(() => [...data.allowed_cidrs]);
  const [trustProxy, setTrustProxy] = useState(() => data.trust_proxy_headers);
          ...
          cidrs.map((cidr, i) => (
            <div key={i} className="flex items-center gap-2">
              ...
              <Button
                ...
                size="icon"
                aria-label="删除网段"
                onClick={() => setCidrs((v) => v.filter((_, idx) => idx !== i))}
              >
