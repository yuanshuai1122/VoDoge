...
主机上的 VoHive 集中管理。不要把每根棒子刷成 Debian 主机：VoHive 的主机才运行
Debian、Docker、PostgreSQL 和一个 VoHive 实例。
...
test -n "$USB_PATH"
CTRL="$(find "$USB_PATH" -path '*/usbmisc/cdc-wdm*' -printf '/dev/%f\n' | head -n 1)"
test -n "$CTRL"
qmicli -d "$CTRL" --dms-get-ids
```

出现 `05c6:90b4`、只有 `rndis*` 网卡或没有 `/dev/cdc-wdm*` 时，当前棒子不应添加到
VoHive 的 QMI 后端。RNDIS/CPE 模式只能作为恢复路径；它们通常共享相同私网地址，且
...
2. 依照 [DEPLOY.md](../DEPLOY.md) 启动唯一的 VoHive Compose 实例；不要为同一 Hub 起第二个
   实例。
3. 在 VoHive 发现页确认该设备的后端是 QMI，添加唯一设备 ID 与 IMEI，再创建一条测试代理。
...
挂载。一个 VoHive 实例应独占整台 Hub，而不是用多个容器争抢同一批设备。

在 VoHive 的设备发现页确认每根棒子都显示为 QMI，并用唯一 IMEI 添加设备，后端选
`qmi`。不要持久化 `/dev/cdc-wdmN` 或 `wwanN`：热插拔会改变这些路径，VoHive 会以 IMEI
重新关联设备。

验证阶段停用 ModemManager，避免它与 VoHive 同时打开 QMI/串口节点：
...
- 当前 VoHive 代码硬限制为 **5 台**。第 6 根起必须先调整设备上限及热插拔测试，不能
  通过启动多个 VoHive 容器绕过。
...
经过验证的完整 Debian 固件组合。它不是把 VoHive 接到多根原厂棒子的必要步骤，也不应
作为批量部署前置条件。
