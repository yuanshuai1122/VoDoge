...
1. `admin` 可登录真实后端，刷新保持登录，改密后正确要求重新登录
2. 六个一级菜单可进入
3. 短信可收（游标分页）、可发、可查投递状态
4. 设备可列表、可看详情，9 种生命周期状态均有明确展示
5. 概览 SSE 实时刷新，断线可自动重连
6. eSIM 至少列表可用，且并发冲突不产生错误状态
7. `make ci` 通过；镜像可 `docker compose up` 起来并访问 UI
