...
| v0.3 的假设 | 实际情况 | 影响 |
|-------------|----------|------|
| 以 `openapi.vohive.yaml` 为接口依据 | spec 缺 17 条真实端点、含 3 条不存在端点 | **不能用 spec 生成类型**，否则漏功能 + 生成死代码 |
...
| # | 标准 | 状态 |
|---|------|------|
| 1 | `admin` 可登录真实后端，刷新保持登录，改密后要求重新登录 | ⬜ **待联调**（代码已实现，未对真实后端验证） |
| 2 | 六个一级菜单可进入 | ✅ 已实现（登录页已实际渲染验证） |
| 3 | 短信可收（游标分页）、可发、可查投递状态 | ⬜ **待联调** |
| 4 | 设备可列表、可看详情，9 种生命周期状态均有明确展示 | ✅ 状态映射已实现；⬜ 真实数据待联调 |
| 5 | 概览 SSE 实时刷新，断线可自动重连 | ⬜ **待联调**（重连交由浏览器） |
| 6 | eSIM 列表可用，并发冲突不产生错误状态 | ⬜ **待联调**（ESIM_BUSY 调度已实现） |
| 7 | `make ci` 通过；镜像可 `docker compose up` 并访问 UI | ⬜ **未验证**（本机无 Go/Docker） |
| 8 | 计划 / 进度 / 决策 / API 矩阵四份文档同步 | ✅ |

**已在本机验证的**：`typecheck` / `eslint --quiet` / `build`（9 路由静态导出）全部通过；
`next dev` 下登录页正确渲染、未登录访问根路径被守卫重定向、后端不可用时错误被
归一化为可读文案。

**未验证的**：任何依赖真实后端返回值的行为。本机无 Go 工具链，无法启动服务端，
因此所有 DTO 字段虽逐一对照源码写出，但**没有一条经过真实响应验证**。
联调时优先用 `scripts/smoke-api.mjs` 快速定位契约偏差。
