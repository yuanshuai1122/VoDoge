...
container_hygiene() {
	...
	if ! git grep -nF 'image: ${VOHIVE_IMAGE:-ghcr.io/yuanshuai1122/vohive:latest}' -- "$compose" >/dev/null; then
		printf 'prebuilt compose file must default to ghcr.io/yuanshuai1122/vohive:latest\n' >&2
		return 1
	fi
